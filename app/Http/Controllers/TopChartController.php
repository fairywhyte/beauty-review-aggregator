...
class TopChartController extends Controller
{
    public function get_five_star_rated_80_percent_products() {
        $products = Product::where('five_star_rating_percentage', '>', 0.79)->orderBy('total_number_of_ratings', 'DESC')->get();
    ...
    public function get_highest_average_rating() {
        $products = Product::where('total_number_of_ratings', '>', 50)->orderBy('average_rating', 'DESC')->get();
        return view('results', ['products' => $products]);
    }

    public function get_all() {
        $products = Product::orderBy('total_number_of_ratings', 'DESC')->get();
        return view('results', ['products' => $products]);
    }
}
